...
class ImageAPIHelper(test_utils.BaseTestCase):
    ...
    def create_and_verify_image(self, name, type=None, disk_format=None,
                                ...
                                headers=None, verify_stores=False,
                                available_stores=[]):
        ...
        if verify_stores:
            ...
            for store in available_stores:
                self.assertIn(
                    store, response.headers['OpenStack-image-store-ids'])
    ...
    def import_image(self, image_id, data=None, headers=None):
        path = f'/v2/images/{image_id}/import'
        if not headers:
            headers = self._headers(
                {'content-type': 'application/json', 'X-Roles': 'admin'})
        response = self.api_post(path, headers=headers, json=data)
        self.import_reqid = response.headers['X-Openstack-Request-Id']
    ...
    def verify_image_import_status(self, image_id, data=None):
        ...
        func_utils.wait_for_status(self, request_path=path,
                                   ...
                                   status='active', max_sec=40, delay_sec=0.2)
    ...
    def verify_image_stores(self, image_id, expected_stores=[],
                            rejected_stores=[]):
        ...
        actual_stores = jsonutils.loads(response.text)['stores']

        # Check that all expected stores are in the actual stores
        for store in expected_stores:
            self.assertIn(store, actual_stores)

        # Check that rejected stores are not in actual stores
        for store in rejected_stores:
            self.assertNotIn(store, actual_stores)
    ...
    def upload_and_verify(self, image_id, image_data, headers=None):
